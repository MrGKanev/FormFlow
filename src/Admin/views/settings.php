...
<?php if ($activeTab === 'general'): ?>
    <section class="panel setup-panel">
        ...
        <ul class="setup-list">
            ...
            <li><span><strong>Turnstile</strong><small>Secret validates submissions; site key appears in snippets</small></span><span class="badge <?= htmlspecialchars($setupBadge($setupStatus['turnstile'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($setupStatus['turnstile'] ?? 'Optional', ENT_QUOTES, 'UTF-8') ?></span></li>
...
<?php if ($activeTab === 'protection'): ?>
    <form method="POST" action="/admin/settings" class="settings-form">
        ...
        <div class="settings-grid">
            <section class="panel"><div class="section-heading"><h2>Cloudflare Turnstile</h2></div><div class="form-grid"><label class="span-2"><span>Secret</span><input type="text" name="turnstile_secret" value="<?= $value('turnstile_secret') ?>"></label><label class="span-2"><span>Site key</span><input type="text" name="turnstile_site_key" value="<?= $value('turnstile_site_key') ?>"></label></div></section>
