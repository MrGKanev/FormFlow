...
<?php if ($activeTab === 'maintenance'): ?>
    <div class="settings-grid">
        ...
        <section class="panel"><div class="section-heading"><h2>Backup</h2></div><div class="form-actions"><form method="POST" action="/admin/backup" class="utility-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>"><button type="submit" class="button secondary">Download database</button></form><form method="POST" action="/admin/config/export" class="utility-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>"><button type="submit" class="button secondary">Export config</button></form></div></section>
